...
from datamodules.preprocess_2d_dataset_patch_based import \
    PreprocessPatchBasedDataset


def create_csv(input_images, input_seg, input_model, output_csv, ndim, min_cell_size):
    dict_path = input_model
    path_output = output_csv
    path_Seg_result = input_seg
    ds = PreprocessPatchBasedDataset(
        path=input_images,
    ...
    ds.preprocess_write_csv(path_to_write=path_output, dict_path=dict_path)
