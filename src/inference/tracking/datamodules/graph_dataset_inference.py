...
class CellTrackDataset:
    def __init__(self,
        ...
        self.__mul_vals = mul_vals
        assert ndim in [2, 3], "Please set dimension 2 or 3."
        self.__is_3d = ndim == 3
        flag_Hela = 'hela' in exp_name.lower()
        self.__filter_edges = filter_edges or flag_Hela
        # attributes for visualization
        self.__debug_visualization = debug_visualization
        # attributes for nodes features
        self.__drop_feat = list(drop_feat)
        self.__normalize = normalize
        self.__which_preprocess = which_preprocess
        self.__separate_models = separate_models
        # attributes for edges features
        self.__edge_feat_embed_dict = edge_feat_embed_dict
        # attributes for both nodes and edges features
        self.__normalize_all_cols = normalize_all_cols
        # attributes for GT construction
        self.__produce_gt = produce_gt
        self.__one_hot_label = one_hot_label
        # attributes for paths handling
        self.__dirs_path = dirs_path
        self.__modes = ["train", "valid", "test"]
        self.__type_file = type_file
        # attributes for graph construction
        self.__same_frame = same_frame
        self.__next_frame = next_frame
        self.__self_loop = self_loop
        self.__overlap = overlap
        self.__directed = directed
        self.__num_frames = num_frames
        self.__jump_frames = jump_frames
        if self.__jump_frames > 1:
    ...
    def true_links(self, df_data):
        ...
        for id in np.unique(df_data.id.values):
            ...
            for ind_node in range(0, nodes.shape[0] - 1):
                ...
                if frames[ind_node] + self.__jump_frames == frames[ind_node + 1]:
    ...
    def _filter_by_roi(self, df_data_curr, df_data_next):
        ...
        for ind in df_data_curr_ceter.index.values:
            ...
            max_row, min_row = row_coord + self.__curr_roi['row'], row_coord - self.__curr_roi['row']
            max_col, min_col = col_coord + self.__curr_roi['col'], col_coord - self.__curr_roi['col']
            ...
            if self.__is_3d:
                ...
                max_depth, min_depth = depth_coord + self.__curr_roi['depth'], depth_coord - self.__curr_roi['depth']
    ...
    def _same_next_links(self, df_data, link_edges):
        """
        ...
        for loop_ind, frame_ind in enumerate(iter_frames[:-1]):
            ...
            if self.__next_frame:
                if frame_ind != iter_frames[-1]:
                    ...
                    if self.__filter_edges:
                        curr_list = self._filter_by_roi(df_data.loc[mask_frame, :], df_data.loc[mask_next_frame, :])
    ...
    def iterator_gt_creator(self, df_data):
        ...
        for ind in range(frames.shape[0] - 1):
            ...
            gt.append(self.__create_gt(df_data[mask_frames], curr_frame, next_frame))
    ...
    def create_gt(self, df_data, curr_frame, next_frame):
        ...
        if self.__one_hot_label:
            y = one_hot(y, num_classes=num_classes).flatten()
        return y

    def _preprocess(self, dropped_df):
        array = dropped_df.values
        if self.__normalize:
            array = self._normalize_array(array)
        return array
    ...
    def _normalize_array(self, array):
        ...
        """
        if self.__which_preprocess == 'MinMax':
            scaler = MinMaxScaler()
        elif self.__which_preprocess == 'Standard':
            scaler = StandardScaler()
        else:
            scaler = MinMaxScaler()
        # array[:, self.__normalize_cols] = scaler.fit_transform(array[:, self.__normalize_cols])
        if self.__separate_models:
            array = scaler.fit_transform(array)
        else:
            array[:, self.__normalize_cols] = scaler.fit_transform(array[:, self.__normalize_cols])
        return array

    def _edge_feat_embedding(self, x, edge_index):
        src, trg = edge_index
        sub_x = x[src] - x[trg]
        abs_sub = np.abs(sub_x)
        res = abs_sub ** 2 if self.__edge_feat_embed_dict['p'] == 2 else abs_sub
        # try to preprocess edge features embedding - min-max normalization or z-score normalization ...
        if self.__edge_feat_embed_dict['normalized_features']:
            res = self._normalize_array(res)
        return res

    def _bb_roi(self, df_data):
        if self.__is_3d:
        ...
        self.__curr_roi = {'row': max_row * self.__mul_vals[0], 'col': max_col * self.__mul_vals[1]}
        if self.__is_3d:
            max_depth = np.abs(bb_feat.min_depth_bb.values - bb_feat.max_depth_bb.values).max()
            self.__curr_roi['depth'] = max_depth * self.__mul_vals[2]
    ...
    def move_roi(self, df_data, curr_dir):
        ...
        self.__curr_roi = {'row': diff_row.max() + self.__mul_vals[0] * diff_row.std(),
                         'col': diff_col.max() + self.__mul_vals[1] * diff_col.std()}
        if self.__is_3d:
            diff_depth = np.abs(df_stats.diff_depth.values)
            self.__curr_roi['depth'] = diff_depth.max() + self.__mul_vals[2] * diff_depth.std()

    def _find_roi(self, files, curr_dir):
        temp_data = [pd.read_csv(file) for file in files]
        df_data = pd.concat(temp_data, axis=0).reset_index(drop=True)
        self._bb_roi(df_data)

    def _create_graph(self, curr_dir, mode):
        """
        ...
        files = [osp.join(curr_dir, f_name) for f_name in sorted(os.listdir(curr_dir)) if
                 self.__type_file in f_name]
        print(f"Start with {curr_dir}")
        num_files = len(files)
        self._find_roi(files, curr_dir)

        if self.__num_frames == 'all':
            num_frames = num_files
        elif isinstance(self.__num_frames, int):
            num_frames = self.__num_frames
        else:
            assert False, f"The provided num_frames {type(self.__num_frames)} variable type is not supported"

        for ind in range(0, num_files, self.__overlap):
            ...
            temp_data = [pd.read_csv(files[ind_tmp]) for ind_tmp in range(ind, ind + num_frames, self.__jump_frames)]
            df_data = pd.concat(temp_data, axis=0).reset_index(drop=True)

            link_edges = []
            if self.__same_frame or self.__next_frame:
                link_edges += self._same_next_links(df_data, link_edges)
            ...
            if is_first_time:
                ...
                if self.__normalize_all_cols:
                    self.__normalize_cols = np.ones((dropped_df.shape[-1]), dtype=bool)
                else:
                    self.__normalize_cols = np.array(
                        ['feat' != name_col[:len('feat')] for name_col in dropped_df.columns])

                if self.__separate_models:
                    self.__separate_cols = np.array(['feat' != name_col[:len('feat')] for name_col in dropped_df.columns])


            if not self.__separate_models:
                x = self._preprocess(dropped_df)
                if self.__edge_feat_embed_dict['use_normalized_x']:
                    edge_feat = self._edge_feat_embedding(x, edge_index)
                else:
                    edge_feat = self._edge_feat_embedding(dropped_df.values, edge_index)
                ...
                if not self.__edge_feat_embed_dict['use_normalized_x']:
                    x = torch.FloatTensor(self._preprocess(dropped_df.loc[:, self.__separate_cols]))
                    x_2 = torch.FloatTensor(dropped_df.loc[:, np.logical_not(self.__separate_cols)].values)
                    edge_feat = self._edge_feat_embedding(dropped_df.values, edge_index)
                else:
                    x = self._preprocess(dropped_df.loc[:, self.__separate_cols])
                    x_2 = dropped_df.loc[:, np.logical_not(self.__separate_cols)].values
                    edge_feat = self._edge_feat_embedding(np.concatenate((x, x_2), axis=-1), edge_index)
                    x = torch.FloatTensor(x)
    ...
    def _process(self, curr_mode):
        ...
        self.all_data = {}
        curr_dir = self.__dirs_path[curr_mode]

        if isinstance(curr_dir, str):
            # this is the case that we get one path (str type)
            curr_dir = osp.join(curr_dir, self.__type_file)  # add type of the files for the folder (../{type})

            data_list = self._create_graph(curr_dir, curr_mode)
            ...
            for dir_path in curr_dir:
                curr_dir = osp.join(dir_path, self.__type_file)  # add type of the files for the folder (../{type})
                data_list += self._create_graph(curr_dir, curr_mode)    # concat all dirs graphs
                print(f"Finish process {curr_dir} ({curr_mode})")
            self.all_data[curr_mode] = data_list
        else:
